...
Flux simplifié avec auto-détection complète des tables BC.
"""
...
<style>
    ...
    .card-data { background:#E1F5EE; border-left:4px solid #0F6E56;
                 border-radius:8px; padding:12px 16px; margin-bottom:8px; }
    .card-ref  { background:#EFF6FF; border-left:4px solid #2E6FBF;
                 border-radius:8px; padding:10px 14px; margin-bottom:6px; }
    .card-err  { background:#FAECE7; border-left:4px solid #993C1D;
                 border-radius:8px; padding:12px 16px; margin-bottom:8px; }
...
with tab1:
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # ÉTAPE 1 — Informations de base uniquement
    # ══════════════════════════════════════════════════════════════════════════
    if st.session_state.step == 1:
        ...
        st.caption(
            "Plus besoin de sélectionner la Master Data — "
            "l'outil la détecte automatiquement depuis le fichier."
        )
        ...
        with col1:
            session_name = st.text_input(
                "Nom de la session *",
                placeholder="Ex: ABC Distribution - Migration données - Juin 2026"
            )
        with col2:
            client = st.text_input(
                "Client *",
                placeholder="Ex: ABC Distribution SARL",
                help="En Sprint 2 : liste déroulante des profils clients."
            )
        ...
        with col_btn:
            if st.button("Suivant →", type="primary", use_container_width=True):
                errors = []
                if not session_name.strip(): errors.append("Nom de session obligatoire.")
                if not client.strip(): errors.append("Client obligatoire.")
                if errors:
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # ÉTAPE 2 — Upload + auto-détection
    # ══════════════════════════════════════════════════════════════════════════
    elif st.session_state.step == 2:

        cfg = st.session_state.config
        st.markdown(
            '<div class="step-header">Étape 2 — Upload du fichier & Détection automatique</div>',
            unsafe_allow_html=True
        )
        st.caption(
        ...
        if uploaded:
            ...
            if not parse_result["success"]:
                ...
                c1.metric("Tables de données", summary["nb_data_tables"],
                          help="Tables où le client a saisi ses données")
                c2.metric("Tables de référence", summary["nb_ref_tables"],
                          help="Listes de valeurs BC (pays, conditions paiement...)")
                c3.metric("Lignes de données", summary["total_data_rows"])

                st.markdown("---")

                # ── Tables de données détectées ────────────────────────────
                st.markdown("### 📊 Tables de données détectées")
                st.caption(
                    "Ce sont les tables que l'outil va analyser et contrôler."
                )

                if summary["data_tables"]:
                    for t in summary["data_tables"]:
                        st.markdown(
                            f'<div class="card-data">'
                            f'<span class="tag tag-data">DONNÉES</span>'
                            f'<b>{t["sheet"]}</b> — {t["label"]} '
                            f'(Table BC n°{t["table_id"]}) · '
                            f'<b>{t["rows"]} lignes</b></div>',
                            unsafe_allow_html=True
                        )
                else:
                    st.warning(
                        "Aucune table de données reconnue. "
                        "Vérifiez que le fichier est bien un export BC."
                    )

                # ── Tables de référence ────────────────────────────────────
                with st.expander(
                    f"📋 Tables de référence détectées ({summary['nb_ref_tables']}) "
                    "— utilisées pour la validation croisée (Sprint 5)"
                ):
                    ref_tables = parse_result.get("ref_tables", [])
                    for sheet in ref_tables:
                        meta = parse_result["metadata"].get(sheet, {})
                        rows = parse_result["total_rows"].get(sheet, 0)
                        st.markdown(
                ...
                with col_b:
                    if st.button("← Retour", use_container_width=True):
                        ...
                        st.rerun()
                with col_v:
                    can_validate = bool(summary["data_tables"])
                    if st.button(
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # ÉTAPE 3 — Résultats + prévisualisation
    # ══════════════════════════════════════════════════════════════════════════
        ...
        if validation["is_valid"]:
            st.success(f"**{summary.get('status', '✅ Fichier conforme')}**")
        else:
            st.error(f"**{summary.get('status', '❌ Fichier non conforme')}**")
        ...
        # ── Tables de données validées ─────────────────────────────────────
        data_tables = validation.get("data_tables", [])
        if data_tables:
            st.markdown(f"### 📊 Tables de données ({len(data_tables)})")
        ...
        # ── Prévisualisation ───────────────────────────────────────────────
        if data_tables and parse_result:
            st.markdown("---")
            ...
            for t in data_tables:
                ...
                if df is not None and not df.empty:
                    with st.expander(
                        ...
                        if t["rows"] > 10:
                            st.caption(
                                f"10 premières lignes sur {t['rows']} au total."
                            )

        # ── Résumé des tables de référence ─────────────────────────────────
        ref_tables = validation.get("ref_tables", [])
        if ref_tables:
            with st.expander(
