"""
Page Sessions — Sprint 5 v3 (fix clés dupliquées).
Analyse complète : Axe A + Axe B. Sessions CRUD.
"""
...
# ════════════════════════════════════════════════════════════════════════════
# HELPER — défini AVANT tout appel. uid évite les clés dupliquées entre axes.
# ════════════════════════════════════════════════════════════════════════════
def display_axe_results(axe_result: dict, axe_label: str, uid: str = "x"):
    """
    Affiche les résultats d'un axe de validation avec onglets par table.
    uid : identifiant unique pour éviter les clés Streamlit dupliquées
          (ex: "a" pour Axe A, "b" pour Axe B).
    """
    ...
    for tab, sheet_name in zip(st.tabs(tab_labels), sheet_names):
        with tab:
            ...
            if real_anomalies:
                ...
                # Clé unique : uid + sheet_name normalisé → jamais de doublon
                safe_sheet = sheet_name.replace(" ", "_").replace("/", "_")
                widget_key = f"ms_{uid}_{safe_sheet}"
...
with tab1:
    ...
    # ── Étape 2 ──────────────────────────────────────────────────────────────
        ...
        if uploaded:
            ...
            if not pr["success"]:
                ...
                ref_tables = pr.get("ref_tables", [])
                metadata   = pr.get("metadata", {})
                total_rows = pr.get("total_rows", {})
    ...
    # ── Étape 3 ──────────────────────────────────────────────────────────────
        ...
        for err in val.get("blocking_errors", []): st.error(err)
        ...
        with col_v:
            if val["is_valid"]:
                if st.button(
                    ...
                    with st.spinner("⏳ Axe A — Contraintes BC..."):
                        axe_a = validate_file_axe_a(pr)
                        st.session_state.axe_a_result = axe_a
                    with st.spinner("⏳ Axe B — Références BC..."):
    ...
    # ── Étape 4 ──────────────────────────────────────────────────────────────
        ...
        for cw,val,lbl,col in [
            ...
            (c5, a_total, "🔵 Axe A",    "#534AB7" if a_total>0 else "#64748B"),
            (c6, b_total, "🟢 Axe B",    "#0F6E56" if b_total>0 else "#64748B"),
        ]:
            with cw:
                st.markdown(
                    f'<div class="stat-box">'
                    f'<p class="stat-num" style="color:{col}">{val}</p>'
                    f'<p class="stat-lbl">{lbl}</p></div>',
                    unsafe_allow_html=True
                )
        ...
        if total == 0 and b_info == 0:
            ...
            with rt1:
                ...
                if real:
                    def cs(row):
                        ...
                        return [""] * len(row)
                    st.dataframe(
                        get_anomalies_dataframe(real).style.apply(cs, axis=1),
                        use_container_width=True, hide_index=True,
                        height=min(500, 50 + len(real) * 35)
                    )
                if infos:
                    st.markdown("---")
                    for a in infos:
            ...
            with rt2:
                # uid="a" → clés : ms_a_18_Client, ms_a_23_Fournisseur...
                display_axe_results(axe_a, "Axe A", uid="a")

            with rt3:
                # uid="b" → clés : ms_b_18_Client, ms_b_23_Fournisseur...
                display_axe_results(axe_b, "Axe B", uid="b")
        ...
        with col_s:
            if st.session_state.saved_session_id:
                st.markdown(
                    f'<div class="save-box">✅ <b>Sauvegardée</b><br>'
                    f'<span style="font-size:11px;color:#64748B">'
                    f'{st.session_state.saved_session_id}</span></div>',
                    unsafe_allow_html=True
                )
            else:
                if st.button(
                    ...
                    if ok:
                        ...
                        st.error(f"❌ {res}")
        with col_st:
            if major == 0: st.success("✅ Prêt pour Sprint 6")
        ...
        for s in sessions:
            ...
            with col_info:
                st.markdown(
                    ...
                    f'</p>'
                    f'<p class="session-meta">{an_str}</p>'
                    f'<p class="session-meta">'
            ...
            if st.session_state.edit_session_id == sid:
                ...
                with e1:
                    new_name = st.text_input(
                        "Nom de la session", value=s.get("name",""), key=f"en_{sid}"
                    )
                ...
                with e2:
                    new_notes = st.text_area(
                        "Notes", value=s.get("notes",""), height=100, key=f"eno_{sid}"
                    )
                cs, cc, _ = st.columns([2, 2, 6])
                with cs:
                    if st.button("💾 Enregistrer", key=f"esv_{sid}",
                                 type="primary", use_container_width=True):
            ...
            if st.session_state.confirm_delete_ses == sid:
                ...
                with cy:
                    if st.button("✅ Confirmer", key=f"dy_{sid}", type="primary"):
                        ...
                        if ok:
                            ...
                            st.error(f"❌ {err}")
                with cn:
                    if st.button("❌ Annuler", key=f"dn_{sid}"):
                        st.session_state.confirm_delete_ses = None
                        st.rerun()
